...
/// Run the salus daemon to completion, returning the process exit code.
///
/// Parses the command line, loads configuration, initializes the database and
/// IPC socket, then serves client connections until the process is terminated.
/// Any failure during setup is rendered to an exit code rather than propagated.
#[must_use]
